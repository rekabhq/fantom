class YamlWriter {
  ...
  String write(dynamic yaml) {
    return _convertToYamlString(yaml).trim();
  }

  /// Write a dart structure to a YAML string. [yaml] should be a [Map] or [List].
  String _convertToYamlString(dynamic yaml, {int indent = 0}) {
    String str = '';
  ...
  String _writeList(List yaml, {int indent = 0}) {
    ...
    for (var item in yaml) {
      str +=
          "${_indent(indent)}- ${_convertToYamlString(item, indent: indent + 1)}\n";
    }
  ...
  String _writeMap(Map yaml, {int indent = 0}) {
    ...
    for (var key in yaml.keys) {
      var value = yaml[key];
      str +=
          "${_indent(indent)}${key.toString()}: ${_convertToYamlString(value, indent: indent + 1)}\n";
    }
